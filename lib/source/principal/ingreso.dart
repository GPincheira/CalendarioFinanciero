...
class Ingreso extends StatefulWidget {  //clase con estado
  final EventModel note;              //se crea note que será de tipo EventModel

  const Ingreso({Key key, this.note}) : super(key: key);  //constante Ingreso incluirá una key y un EventModel

  @override
  _IngresoState createState() => _IngresoState();
}

class _IngresoState extends State<Ingreso> { //clase que extiende la anterior
  ...
  DateTime _fecha;
  //se agrega una lista de string con las diferentes categorías que se pueden seleccionar como Ingreso
  List<String> categorias = <String>['Salario', 'Ingreso por venta', 'Devolución de dinero', 'Prestamo solicitado', 'Otro'];
  ...
  void initState() { //initState, metodo utilizado para inicializar algunas variables
    super.initState();
    _tipo = "Ingreso";  //como es un ingreso, el tipo no será modificado por el usuario
    _categoria = "Salario"; //categoria se selecciona de una lista, se inicializa en la primera opción
            ...
            children: <Widget>[
              Padding(  //tiene como hijos diferentes padding con simetría
                ...
                child: Column(  //padding tiene como hijo una columna
                  ...
                  children: <Widget>[
                    Text("Registrar Ingreso", style: TextStyle(fontSize: 28),), //mostrará un texto como título
                  ],
                ),
              ),
